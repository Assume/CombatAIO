...
public class CombatTask extends Threadable implements Pauseable {
	...
	public CombatTask() {
		this(Arrays.asList(
				new PauseType[] { PauseType.COULD_INTERFERE_WITH_LOOTING, PauseType.COULD_INTERFERE_WITH_EATING }));
	...
	public void fight() {
		...
		if (this.shouldChangeWorld() && !Player.getRSPlayer().isInCombat()) {
			...
			int rand_world = WorldHopper.getRandomWorld(true);
			while (rand_world == WorldHopper.getWorld())
				rand_world = WorldHopper.getWorld();
			WorldHopper.changeWorld(rand_world);
		}
	...
	private void executeHasTarget() {
		RSCharacter interacting_char = Player.getRSPlayer().getInteractingCharacter();
	...
	private void fight(RSNPC[] monsters) {
		if ((monsters.length == 0 && NPCs.find(this.monster_ids).length >= 0
				&& Player.getPosition().distanceTo(home_tile) >= combat_distance)
				|| Timing.timeFromMark(this.last_attack_time) >= 10000) {
			if (Player.getPosition().distanceTo(home_tile) >= 5 && (!Dispatcher.get().isRockCrabsPreset()
					&& !(Dispatcher.get().getPreset() != PresetFactory.WATERBIRTH_ROCK_CRABS))) {
	...
	public boolean verifyTarget(RSNPC current_target) {
		...
		return this.attack_monsters_already_in_combat
				|| (!current_target.isInCombat() && !TargetFinder.isBeingSplashed(current_target));
	...
	private void moveToTarget(RSNPC target) {
		...
		if (!target.isOnScreen() || (!this.helper.isRanging() && !PathFinding.canReach(target, false))) {
	...
	public boolean attackCurrentTarget() {
		if (this.current_target != null)
			return Clicking.click("Attack " + this.current_target.getName(), this.current_target);
		return false;
	}
	...
	private void attackTarget(RSNPC target) {
		...
		if (Clicking.click("Attack " + target.getName(), target)) {
			...
			int sleep_time = General.random((int) ((distance / 3.5) * 1000), (int) ((distance / 2.5) * 1000));
	...
	public Value<int[]> getGuthansIDs() {
		return new Value<int[]>(ArrayUtil.combineArrays(CombatHelper.GUTHANS_BODY_IDS, CombatHelper.GUTHANS_HELM_IDS,
				CombatHelper.GUTHANS_LEGS_IDS, CombatHelper.GUTHANS_WARSPEAR_IDS));
	}
