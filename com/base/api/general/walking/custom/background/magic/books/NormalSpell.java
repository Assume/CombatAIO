...
public enum NormalSpell implements Serializable {
   ...
   /* WIND_STRIKE("Wind Strike", 1, new Rune[]{Rune.MIND, Rune.AIR}, new int[]{1, 1}, SpellType.COMBAT),
     ...
     * @param requiredLevel The required level to cast the spell.
     ...
     */
    private NormalSpell(String name, int requiredLevel, Rune[] requiredRunes, int[] numberOfRunes, SpellType type) {
        this.name = name;
    ...
    /**
     * Checks to see if the player has the required runes to cast the NormalSpell.
     * <p>
            ...
            if (children != null && children.length > 0) {
                for (RSInterface child : children) {
                    if (child != null) {
                        ...
                        if (child_name != null && child_name.contains(getName())) {
                            return child;
                        }
                    }
                }
