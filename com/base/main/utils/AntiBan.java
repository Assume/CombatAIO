...
public final class AntiBan {
	...
	public static int getReactionTime() {
		...
		properties
				.setUnderAttack(Combat.isUnderAttack()
						|| (Timing.currentTimeMillis() - last_under_attack_time < 2000));
	...
	public static void resetShouldOpenMenu() {
		should_open_menu = getABCUtil().shouldOpenMenu()
				&& getABCUtil().shouldHover();
	}
	...
	public static boolean shouldSwitchResources(int player_count) {
		...
		return win_percent < 50.0
				&& getABCUtil().shouldSwitchResources(player_count);
	}
					...
					if (name != null) {
						return name.equals(objectName)
								&& !o.getPosition().equals(
										currentlyInteracting.getPosition())
				...
				final RSNPC next = NPCs07.getNPC(new Filter<RSNPC>() {
					...
					public boolean accept(RSNPC npc) {
						...
						return npc.isOnScreen()
								&& npc.isClickable()
								&& !npc.getPosition().equals(
										interacting.getPosition());
					}
					// </editor-fold>
				});
				if (next != null) {
					if (!Entities.isHovering(next.getModel())
							&& Clicking.hover(next)) {
						Timing.waitCondition(new Condition() {
							@Override
							public boolean active() {
	...
	public static void walkToPosition(Positionable location) {
		WalkingPreference pref = AntiBan.getABCUtil()
				.generateWalkingPreference(
						Player.getPosition().distanceTo(location));
