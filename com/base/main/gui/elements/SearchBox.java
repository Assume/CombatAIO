 ...
 * @author Nolan
 * @param <T>
 *            The Listable that is being displayed in this search box.
 */
public class SearchBox<T extends Listable> extends JComboBox<T> {
	...
	public void clear() {
		...
		if (e.getItem().toString()
				.equalsIgnoreCase(getPrompt().getListDisplay())
				|| getEditor().getEditorComponent().getForeground()
						.equals(new Color(150, 150, 150, 200))) {
			e.setItem("");
	...
	public Object getSelectedItem() {
		if (getModel().getSelectedItem() != null
				&& getModel().getSelectedItem() instanceof Listable
				&& !getModel().getSelectedItem().equals(noResults)) {
		...
		return item;
		// return getModel().getSelectedItem() instanceof Listable ?
